...
# @@Created          :  Wednesday, Feb 05, 2025 08:28 EST
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for ampache
...
SERVICE_USER="ampache"  # execute command as another user
SERVICE_GROUP="ampache" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for ampache"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
